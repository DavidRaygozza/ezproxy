...
Copyright (C) Beau Daoust. All rights reserved.
===============================================

---PLEASE do not edit, rename, or delete any of the files in this project unless absolutely necessary.---

Instructions on how to access these project files or the mySQL server:
-
1. Must connect to the univerity of la verne VDI (must disconnect from vpn when doing this)
2. Open 'Remote Desktop Connection' to remotely access the server
3. Server name is LIB-DB, username and password are your individual accoun credentials provided by OIT
4. To access project files click on file app, scroll down to CriticalEzproxyBackend > LogProcessing > venv
5. To access the local mySql server open mySQL Workbench, click localhost and enter password 'Fall2010'
...
1. Upload to MySQL database
    This option allows you to choose CSV files to upload to the database.
    Please use caution when using this feature, as this could result in data redundancy.
2. Change login credentials
    This option allows you to change the usernames and/or passwords for the MySQL database user, EZProxy Admin Access website, and super user.
    This option should only be used when absolutely necessary as it could prevent the project from working properly.
===

Instructions on how to migrate the entire project to a new directory:
-
This is a somewhat tedious process, so hopefully these instructions will be concise enough.
These instructions are written with the assumption that PyCharm is installed on your system.
If it is not already installed, you can download it for free here: https://www.jetbrains.com/pycharm/download/
If you'd like information about PyCharm, I made a video covering the basics here: https://www.youtube.com/watch?v=lzlfyJTCfwQ
1. Open PyCharm and start a new project.
2. After all loading is done, open terminal by clicking the button on the bottom left of PyCharm, and install scrapy in terminal by typing 'pip install scrapy mysql-connector-python json2'.
...
18. Go back into your new task and make sure it is set to 'Run whether user is logged in or not'
19. Disable the old, previous task to make sure only the new task runs everyday at 3:00:00 AM
...
'creds.json'
    This JSON file contains the login credentials for the EZproxy Admin Access site, MySQL database, and super user.
...
(quizk fix: if password for account is ever changed go into task scheduler, double click task, click ok,a nd enter new password for account)
-
...
1. Ensure that all files are accounted for. Refer to 'File Hierarchy (as of June 28 2021).png' in the above directory.
...
If any of the above steps fixed the project, you will now need to upload all missing log data up to yesterday (the day prior to the day you are reading this).
	...
	If there is no one you know with such experience, you may call me (Beau Daoust) for assistance.
		The school should still have my phone number, or you may be able to find it in the 'ezproxy.studentdata' table in the MySQL database (SELECT * FROM ezproxy.studentdata WHERE last_name = 'Daoust';).

Thank you for taking the time to read this document!
As of June 28th 2021, this project was developed entirely by Beau Daoust, ULV alumnus with a B.S. in Computer Science, Software.
